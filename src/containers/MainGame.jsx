...
class MainGame extends Component {
	...
	tick() {
		...
		this.props.incrementCapsaicin.incrementCapsaicin(
			this.props.game,
			this.state.timer
		);
				...
				<div className="card">
					...
					{this.props.upgrades.eligibleUpgrades.map((upgrade) => (
						<button
							...
							type="button"
							onClick={() =>
								this.purchaseUpgrade(this.props.game, upgrade.upgrade)
							}
							key={upgrade.text}
						>
							{upgrade.text}
			...
			<div className="column medium-3">
				<div className="card">
					...
					{game.helpers.eligibleHelpers.map((helper) => {
						...
						if (
							helper.helper === "Madagascar" &&
							game.helpers.purchasedHelpers.Madagascar >= 1
						) {
							return (
			...
			<div className="column medium-3">
				<div className="card">
					...
					<button
						className="button"
						type="button"
						onClick={() => this.onClick(this.props.game)}
					>
						Click Me
					</button>
				</div>
		...
		if (this.state.chat) {
			return (
				<div className="column medium-6 small-12">
					...
					>
						<div
							className="messages grid-y align-right"
							style={{ height: 300 }}
						/>
						<form>
					...
					<div className="card" style={{ height: 0, visibility: "hidden" }}>
						...
						<form data-abide="ajax" id="login-form">
							...
							<input
								...
								value="Login"
								onClick={() =>
									this.handleLogin(this.state.loguser, this.state.logpass)
								}
							/>
							<input
								type="submit"
				...
				<div className="card" style={{ height: 467.94, visibility: "visible" }}>
					<form data-abide="ajax" id="create-form">
						...
						<input
							...
							value="Create User"
							onClick={() =>
								this.handleCreateUser(this.state.username, this.state.password)
							}
						/>
					</form>
					<form data-abide="ajax" id="login-form">
	...
	render() {
		return (
			<div className="grid-x">
				...
				<div
					className="cell medium-6 medium-cell-block-y"
					style={{ height: 1400 }}
				>

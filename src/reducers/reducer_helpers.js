export default function(state, action) {
  let helpers = [
    ...
    {helper: 'Greenhouse', text: 'Cost 100 plants and 4 gardeners, generates .5 plants/sec', progress: 2},
    {helper: 'Farm', text: 'Cost 1000 plants and 3 greenhouses, generates 5 plants/sec', progress: 3},
    {helper: 'Aquaponics', text: 'Cost 100 heat and 5 farms, generates 100 plants/sec', progress: 4},
    ...
    {helper: 'Pepper Forest', text: 'Cost 500 farms and 50,000 heat, generates 50,000 plants/sec', progress: 7},
  ...
  switch(action.type) {
    ...
    case 'BUY_GREENHOUSE': {
      if(action.payload.plants >= 100 && action.payload.helpers.purchasedHelpers.Gardeners >= 4) {
        if(action.payload.upgrades.purchasedUpgrades.includes('universal_click_times_ten') && action.payload.plants >= 1000 && action.payload.helpers.purchasedHelpers.Gardeners >= 40) {
    ...
    case 'BUY_FARM': {
      if(action.payload.plants >= 1000 && action.payload.helpers.purchasedHelpers.Greenhouses >= 3) {
        if(action.payload.upgrades.purchasedUpgrades.includes('universal_click_times_ten') && action.payload.plants >= 10000 && action.payload.helpers.purchasedHelpers.Greenhouses >= 30) {
    ...
    case 'BUY_AQUAPONICS': {
      if(action.payload.helpers.purchasedHelpers.Farms >= 5 && action.payload.capsaicin >= 100) {
        if(action.payload.upgrades.purchasedUpgrades.includes('universal_click_times_ten') && action.payload.helpers.purchasedHelpers.Farms >= 50 && action.payload.capsaicin >= 1000) {
    ...
    case 'BUY_FOREST': {
      if (action.payload.helpers.purchasedHelpers.Farms >= 500 && action.payload.capsaicin >= 50000) {
